    ...
    while (true) {
      ...
      if (sEnemy.length == 0) {
        ...
        const equipment = createEquipment({
          requiredEquipmentLevel: fightStore.getFight.player.playerInfo.level,
        });
      ...
      if (isPlayer) {
        ...
        if (attack.isLife()) {
          ...
          await fightFunction(attack, immortalCultivator, fightList, sEnemy);
          isPlayer = false
        }
        playerFightIndex++;
        playerFightIndex = playerFightIndex % fightList.length;
        fightList.forEach(item => item.endRoud())
      } else {
        const attack = currentEnemy[enemyFightIndex];
        if (attack.isLife()) {
          // 敌人随机选则一个
          const immortalCultivator = randomUtil.pickone(sPlayer);
          await fightFunction(attack, immortalCultivator, currentEnemy, sPlayer);
          isPlayer = true
        }
        enemyFightIndex++;
        enemyFightIndex = enemyFightIndex % currentEnemy.length;
        currentEnemy.forEach(item => item.endRoud())
      }
      round2++;
      if (round2 % 2 == 0) {
...
async function fightFunction(attacker: ImmortalCultivators, defencer: ImmortalCultivators, attackTeam: ImmortalCultivators[], defenderTeam: ImmortalCultivators[]) {
  // 血量计算
  let harm = 0;
  // 技能执行判断
  ...
  if (skillSelectStrategy) {
    ...
    harm = fightTargetAndGetHarm(attacker, defencer);
  }
  // 播放动画
...
function restore(item) {
  item.currentLife = item.getLife();
  item.currentMana = item.getMana();
}
</script>
        ...
        <a-tabs v-model:activeKey="activeKey" type="card">
            ...
            >
              <a-tab-pane v-for="(item,i) in fightStore.getFight.player.getAllList()" :key="i"
                ...
                <a-row>
                  ...
                  <a-col :span="6">
                    <div>
                      <a-button @click="restore(item)">恢复</a-button>
                    </div>
          ...
          <a-tab-pane key="2" tab="背包" force-render>
            <a-row>
              ...
              <a-col
                  v-for="item in fightStore.getFight.player.getAllList()[characterIndex].getBackpackCapacity()-fightStore.getFight.player.getAllList()[characterIndex].baseEquipment.length"
                  :key="item">
                <a-card
                    style="width: 60px; height: 60px;margin: 5px"
                    :bodyStyle="{ margin: '0', padding: '0',border:'#1a1a1a 1px solid',borderRadius: '5px' }"
